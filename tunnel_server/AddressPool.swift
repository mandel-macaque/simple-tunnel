...
class AddressPool {
	...
	init(startAddress: String, endAddress: String) {
        ...
        simpleTunnelLog("Address pool init \(startAddress) - \(endAddress)")
        ...
        simpleTunnelLog("Address pool init ended")
	}

	/// Allocate an address from the pool.
	func allocateAddress() -> String? {
		var result: String?
        simpleTunnelLog("allocateAddress")
        queue.sync() {
	...
	func deallocateAddress(addrString: String) {
        simpleTunnelLog("deallocateAddress")
        queue.sync() {
