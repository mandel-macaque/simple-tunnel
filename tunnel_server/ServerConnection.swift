...
class ServerConnection: Connection, StreamDelegate {
	...
	func open(host: String, port: Int) -> Bool {
        ...
        simpleTunnelLog("Connection opened")
		return true
	}
    ...
    override func closeConnection(_ direction: TunnelConnectionCloseDirection) {
        ...
        simpleTunnelLog("Close connection")
    ...
    override func abort(_ error: Int = 0) {
        simpleTunnelLog("Connection aborted")
		super.abort(error)
		closeConnection(.all)
	}
	...
	override func suspend() {
        simpleTunnelLog("Connection suspended")
	...
	override func resume() {
        simpleTunnelLog("Connection resumed")
		if let stream = readStream {
            stream.schedule(in: .main, forMode: RunLoopMode.defaultRunLoopMode)
		}
	}
    ...
    override func sendData(_ data: Data) {
        simpleTunnelLog("Connection data sent")
    ...
    func stream(aStream: Stream, handleEvent eventCode: Stream.Event) {
        simpleTunnelLog("Connection Handle event on a stream")
		switch aStream {
