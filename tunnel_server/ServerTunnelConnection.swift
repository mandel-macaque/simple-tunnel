...
class ServerTunnelConnection: Connection {
	...
	func createTUNInterface() -> Int32 {
        simpleTunnelLog("createTUNInterface")
       ...
       let connectResult = withUnsafePointer(to: &socketAddressControl) { sockAddr in
	...
	func getTUNInterfaceName(utunSocket: Int32) -> String? {
        simpleTunnelLog("getTUNInterfaceName")
	...
	func startTunnelSource(utunSocket: Int32) {
        simpleTunnelLog("startTunnelSource")
        ...
        let newSource = DispatchSource.makeReadSource(fileDescriptor: 0, queue: .main)
    ...
    override func closeConnection(_ direction: TunnelConnectionCloseDirection) {
        simpleTunnelLog("closeConnection")
	...
	override func suspend() {
        simpleTunnelLog("suspend")
		isSuspended = true
		if let source = utunSource {
			source.suspend() // dispatch_suspend(source)
		}
	}
	...
	override func resume() {
		...
		isSuspended = false
		if let source = utunSource {
			source.resume()// dispatch_resume(source)
			readPackets()
		}
	}
    ...
    override func sendPackets(_ packets: [Data], protocols: [NSNumber]) {
        ...
        for (index, packet) in packets.enumerated() {
            ...
            packet.withUnsafeBytes() { (bytes:UnsafePointer<UInt8>) -> Void in
                let buffer = UnsafeMutableRawPointer(mutating: bytes)
                ...
                if writeCount < 0 {
                    //if var errorString = String(cString: strerror(errno)) {
                    var errorString = String(cString: strerror(errno))
